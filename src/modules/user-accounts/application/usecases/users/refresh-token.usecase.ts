...
export class RefreshTokenUseCase implements ICommandHandler<RefreshTokenCommand> {
  ...
  async execute({ dto }: RefreshTokenCommand): Promise<{ accessToken: string, refreshToken: string }> {
    ...
    const user = await this.usersQueryRepository.findById( dto.userId );
    if ( !user ) {
    ...
    const device = await this.securityDeviceRepository.findByDeviceIdAndUserId({ deviceId: dto.deviceId, userId: dto.userId });
    if (!device) {
      throw new DomainException({
